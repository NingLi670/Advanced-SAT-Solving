...
# NOTE: `up_idx` is for recording which assignment triggers the BCP
def bcp(sentence, assignment, c2l_watch, l2c_watch, up_idx=0):
    ...
    while up_idx < len(assignment):
        ...
        for clause_idx in watching_clause_idxs:
            if len(sentence[clause_idx]) == 1:
                return sentence[clause_idx]

            another_lit = c2l_watch[clause_idx][0] if c2l_watch[clause_idx][1] == - \
                lit else c2l_watch[clause_idx][1]
...
# NOTE: `assignment` is for filtering assigned literals
...
# NOTE: `sentence` is for resolution
def analyze_conflict(sentence, assignment, decided_idxs, conflict_ante):
    ...
    while True:
        ...
        while not is_resolved:
            ...
            if -lit in learned_clause:
                learned_clause = list(
                    set(learned_clause + sentence[clause_idx]))
    ...
    if len(learned_clause) == 1:
        ...
        backtrack_level = next((level for level, assigned_idx in enumerate(
            decided_idxs) if assigned_idx > second_highest_assigned_idx), 0)
...
def cdcl(sentence, num_vars):
    ...
    while len(assignment) < num_vars:
        ...
        # Run BCP.
        conflict_ante = bcp(sentence, assignment, c2l_watch,
                            l2c_watch, len(assignment) - 1)
        while conflict_ante is not None:
            # Learn conflict.
            backtrack_level, learned_clause = analyze_conflict(
                sentence, assignment, decided_idxs, conflict_ante)
            ...
            backtrack(assignment, decided_idxs,
                      backtrack_level, learned_clause)

            # Propagate watch.
            conflict_ante = bcp(sentence, assignment,
                                c2l_watch, l2c_watch, len(assignment))
