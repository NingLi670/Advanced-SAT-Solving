...
class CDCL_SOLVER:
    ...
    def mlr_after_conflict(self, learned_clause):
        ...
        if self.conflict > 3:
            ...
            self.mlr_v = self.mlr_beta_2 * \
                self.mlr_v + (1-self.mlr_beta_2) * g * g
            m_hat = self.mlr_m / (1 - self.mlr_beta_1**self.mlr_t)
            v_hat = self.mlr_v / (1 - self.mlr_beta_2**self.mlr_t)
            self.mlr_theta = self.mlr_theta - self.mlr_alpha * \
                m_hat / (np.sqrt(v_hat)+self.mlr_epsilon)
    ...
    def after_conflict_analysis(self, learned_clause, conflict_side, reasons):
        ...
        unassigned_lits = np.in1d(
            np.arange(-self.num_vars, self.num_vars + 1), assigned_lits, invert=True)
        self.q_v[unassigned_lits] = 0.95 * self.q_v[unassigned_lits]

    # chb
    def update_q_v(self):
        plays = np.asarray(list(self.assigned_lits))
        reward = self.multi / (self.num_conflicts -
                               self.assigned_v[plays+self.num_vars]+1)
        self.q_v[plays + self.num_vars] = (1.-self.alpha) * \
            self.q_v[plays+self.num_vars]+self.alpha*reward
    ...
    def decide_vsids(self):  # NOTE: `assignment` is for filtering assigned literals
        ...
        for lit in assigned_lits:
            ...
            unassigned_vsids_scores[-lit] = float("-inf")
        assigned_lit = max(unassigned_vsids_scores,
                           key=unassigned_vsids_scores.get)
        return assigned_lit
    ...
    def analyze_conflict(self, conflict_ante):  # NOTE: `sentence` is for resolution
        ...
        lit_to_assigned_idx = {
            lit: assigned_lits.index(-lit) for lit in learned_clause}
            ...
            while conflict_ante is not None:
                ...
                if self.restart == 1:
                    ...
                    if self.mlr_after_BCP(is_conflict):
                        ...
                        # print("assignment_len:", len(self.assignment),
    ...
    def run(self):
        if self.restart == 1:
            if self.MAB == 1 or self.MAB == 2:
                print(
                    "Run solver with restarting method and choosing heuristic with UCB: ", self.UCB)
                return self.run_restart_UCB()
            else:
                print(
                    "Run solver with restarting method and choosing heuristic : ", self.solver)
                return self.run_restart()
        else:
            print(
                "Run solver without restarting method and choosing heuristic : ", self.solver)
    ...
    def update_MOSS(self):
        ...
        self.MOSS[0] = self.reward[0] + \
            np.sqrt((4/self.n[0]) * np.log(np.max([1, (self.t / n0)])))
        self.MOSS[1] = self.reward[1] + \
            np.sqrt((4/self.n[1]) * np.log(np.max([1, (self.t / n1)])))
        self.MOSS[2] = self.reward[2] + \
            np.sqrt((4/self.n[2]) * np.log(np.max([1, (self.t / n2)])))
