...
class CDCL_LRB:
    def __init__(self, sentence, num_vars):
        ...
        self.assigned_lits_set = set()
        self.assigned_lits_list = []
        self.unassigned_lits = set(range(-self.num_vars, self.num_vars+1))
        self.k = 0
    ...
    def on_unassign(self, literals):
        ...
        # print(assigned_lits)
        interval = self.LearntCounter - \
            self.assigned_v[assigned_lits+self.num_vars]
        # print(interval)
        r = self.participated_v[assigned_lits +
                                self.num_vars] / (interval+0.00001)
        rsr = self.resoned_v[assigned_lits +
                             self.num_vars] / (interval+0.00001)
        self.q_v[assigned_lits + self.num_vars] = (
            1.0 - self.alpha) * self.q_v[assigned_lits + self.num_vars] + self.alpha * (r + rsr)
    ...
    def bcp(self, up_idx=0):
        ...
        if len(self.assignment) == 0:
            ...
            for clause_idx, watched_lits in self.c2l_watch.items():
                if len(watched_lits) == 1 and watched_lits[0] not in self.assigned_lits_set and -watched_lits[0] not in self.assigned_lits_set:
                    self.assigned_lits_set.add(watched_lits[0])
                    self.assigned_lits_list.append(watched_lits[0])
                    self.unassigned_lits.remove(watched_lits[0])
                    self.unassigned_lits.remove(-watched_lits[0])
        ...
        # we use `up_idx = len(assignment)` to indicate after-conflict BCP
        elif up_idx == len(self.assignment):
            neg_first_uip = self.sentence[-1][-1]
            self.assignment.append((neg_first_uip, len(self.sentence) - 1))
            self.assigned_lits_set.add(neg_first_uip)
            self.assigned_lits_list.append(neg_first_uip)
            self.unassigned_lits.remove(neg_first_uip)
            self.unassigned_lits.remove(-neg_first_uip)
        ...
        while up_idx < len(self.assignment):
            ...
            for clause_idx in watching_clause_idxs:
                if len(self.sentence[clause_idx]) == 1:
                    return self.sentence[clause_idx]

                another_lit = self.c2l_watch[clause_idx][0] if self.c2l_watch[clause_idx][1] == - \
                    lit else self.c2l_watch[clause_idx][1]
                if another_lit in self.assigned_lits_set:
                    continue

                is_new_lit_found = False
                for tmp_lit in self.sentence[clause_idx]:
                    if tmp_lit != -lit and tmp_lit != another_lit and -tmp_lit not in self.assigned_lits_set:
                ...
                if not is_new_lit_found:
                    if -another_lit in self.assigned_lits_set:
                        # NOTE: return a clause, not the index of a clause
                        return self.sentence[clause_idx]
                    else:
                        self.assigned_lits_set.add(another_lit)
                        self.assigned_lits_list.append(another_lit)
                        self.unassigned_lits.remove(another_lit)
                        self.unassigned_lits.remove(-another_lit)
    ...
    def decide_q_v(self):  # NOTE: `assignment` is for filtering assigned literals
        ...
        assigned_lits = np.asarray(self.assigned_lits_list)
    ...
    def init_watch(self):
        ...
        # for each clause watch first two literals
        for clause_idx, clause in enumerate(self.sentence):
    ...
    def analyze_conflict(self, conflict_ante):  # NOTE: `sentence` is for resolution
        ...
        while True:
            ...
            lits_at_conflict_level = self.assigned_lits_list[self.decided_idxs[-1]:]
            clause_lits_at_conflict_level = [
                -lit for lit in learned_clause if -lit in lits_at_conflict_level]
            ...
            while not is_resolved:
                lit, clause_idx = self.assignment[-back_index]
                # if lit == lit1 and clause_idx == clause_idx1: print(True)
                back_index += 1
                if -lit in learned_clause:
                    reasons.update(self.sentence[clause_idx])
                    # reasons = list(set(reasons + self.sentence[clause_idx]))
                    learned_clause = learned_clause.union(
                        set(self.sentence[clause_idx]))
        ...
        lit_to_assigned_idx = {
            lit: self.assigned_lits_list.index(-lit) for lit in learned_clause}
        learned_clause = sorted(
            learned_clause, key=lambda lit: lit_to_assigned_idx[lit])
        ...
        # print('backtolevel: '+str(backtrack_level))
    ...
    def after_conflict_analysis(self, learned_clause, conflict_side, reasons):
        ...
        # print(tmp1)
        self.participated_v[tmp1 +
                            self.num_vars] = self.participated_v[tmp1 + self.num_vars] + 1.
    ...
    def lrb_decay(self):
        ...
        self.q_v[unassigned_lits+self.num_vars] = 0.95 ** self.k * \
            self.q_v[unassigned_lits+self.num_vars]
        self.k = 0

    def backtrack(self, level):
        """Backtrack by deleting assigned variables."""
        literals = self.assigned_lits_list[self.decided_idxs[level]:]
        self.on_unassign(literals)
        del self.assignment[self.decided_idxs[level]:]
        del self.assigned_lits_list[self.decided_idxs[level]:]
        del self.decided_idxs[level:]
        for lit in literals:
            self.assigned_lits_set.remove(lit)
    ...
    def run(self):
        ...
        while len(self.assignment) < self.num_vars:
            ...
            self.assigned_lits_set.add(assigned_lit)
            self.assigned_lits_list.append(assigned_lit)
            self.unassigned_lits.remove(assigned_lit)
            self.unassigned_lits.remove(-assigned_lit)
            ...
            while conflict_ante is not None:
                ...
                # Learn conflict.
                # time1 = time.time()
                backtrack_level, learned_clause, conflict_side, reasons = self.analyze_conflict(
                    conflict_ante)
                # time2 = time.time()
                # print('analyze_conflict time: ' + str(time2 - time1))
                if backtrack_level != -1:
                    self.after_conflict_analysis(
                        learned_clause, conflict_side, reasons)
                # time3 = time.time()
